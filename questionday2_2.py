...
def nearest_corners(S,A):
    A = A.split()
    desk_index = A.index(S)
    gap_index = A.index('-')
    if desk_index == gap_index - 1 or desk_index == gap_index+1:
        return 0
    else:
        return min(abs(desk_index - gap_index - 1), abs(desk_index - gap_index -1))
S = input()
A = input()
print(nearest_corners(S,A))
